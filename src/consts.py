...
SLACK_CALLBACK_ID_LIMIT = 255
SLACK_MAX_PAGINATION_PAGES = 1000
SLACK_DEFAULT_TIMEOUT = 30
SLACK_REST_CALL_TIMEOUT = 30
...
SLACK_CONNECTIONS_OPEN = "apps.connections.open"
...
SLACK_ERROR_NEGATIVE_INT = (
    "Please provide a valid non-negative integer value in the {key} parameter"
)
SLACK_ERROR_NEGATIVE_AND_ZERO_INT = (
    "Please provide a valid non-zero positive integer value in the {key} parameter"
)
...
SLACK_ERROR_PARSE_JSON_FROM_CALLBACK_ID = (
    "Could not parse JSON from callback_id field in payload: {error}"
)
...
SLACK_ERROR_COULD_NOT_OPEN_ANSWER_FILE = (
    "Could not open answer file for writing: {error}"
)
SLACK_ERROR_WHILE_WRITING_ANSWER_FILE = (
    "Error occurred while writing in answer file: {error}"
)
...
SLACK_ERROR_USER_TOKEN_NOT_PROVIDED = "'OAuth Access Token' is required for this action. Navigate to the asset's configuration and add a token now and try again."
...
SLACK_ERROR_MESSAGE_TOO_LONG = (
    "Message too long. Please limit messages to {limit} characters."
)
SLACK_ERROR_QUESTION_TOO_LONG = (
    "Question too long. Please limit questions to {limit} characters."
)
SLACK_ERROR_CONFIRMATION_TOO_LONG = (
    "Confirmation too long. Please limit questions to {limit} characters."
)
...
SLACK_ERROR_FILE_OR_CONTENT_NOT_PROVIDED = (
    "Please provide either a 'file' or 'content' to upload"
)
...
SLACK_INGESTION_NOT_ENABLED = (
    "{message} Ingestion isn't enabled, not going to disable it."
)
...
SLACK_ERROR_SLACKBOT_RUNNING_WITH_SAME_BOT_TOKEN = "Detected an instance of SlackBot running with the same bot token. Not going to start new instance."
SLACK_SUCCESSFULLY_SLACKBOT_STARTED = "SlackBot started"
SLACK_ERROR_UNABLE_TO_SEND_QUESTION_TO_CHANNEL = "Questions can only be sent as direct messages to users using this action. To send message in channel, use the ask question channel action."
SLACK_ERROR_UNABLE_TO_SEND_QUESTION_TO_USER = "Questions can only be sent in channel using this action. To send message to a user, use the ask question action."
...
SLACK_ERROR_INVALID_CHANNEL_TYPE = (
    "Please provide a valid value in the 'channel_type' action parameter"
)
SLACK_ERROR_QUESTION_TIMED_OUT = "Question timed out with no response"
SLACK_ERROR_LENGTH_LIMIT_EXCEEDED = "Based on your asset_id length ({asset_length}), valid length for the 'confirmation' parameter is {valid_length}"
SLACK_ERROR_BLOCKS_OR_MESSAGE_REQD = (
    "You must provide at least one of 'blocks' or 'message'"
)
...
SLACK_ERROR_NO_CHANNEL_ID = (
    "No Channel ID was provided. Please provide a Channel ID for the target channel."
)
...
SLACK_SUCCESSFULLY_CONVERSATION_HISTORY_DATA_RETRIEVED = (
    "Conversation history data successfully retrieved"
)
